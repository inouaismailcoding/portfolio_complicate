...
class Table_MYSQL  { //extends Mysql_DB
    ...
    public function create($data,$file) {
        return $this->DB->create_row($this->table, $data,$file);
    }
    public function update($data,$id) {
        return $this->DB->update_row($this->table, $data,$id);
    }
    public function destroy($id) {
        return $this->DB->destroy_row($this->table, $id);
    }
    public function all() {
    ...
    public function describe(){
        return $this->DB->describeTable($this->table)->data; 
    }
    ...
    public function getFields() {
        ...
        foreach ($desc as $field) {
            ...
            $this->attrsBlock[$field->Field] = [];
            //$this->fieldFormBlock[$field->Field] = [];
        }
    }
    ...
    private function initializeFieldForms() {
        foreach ($this->_fields as $field) {
            if ($field->Field !== 'id') {
                ...
                $this->fieldForm[$field->Field] = $this->generateInputField($attrs, strtok($field->Type, '('), $this->isRequired($field->Null));
                $this->fieldFormBlock[$field->Field]=$this->generateBlockField($field->Field);
            }
        }
    }
    ...
    private function generateBlockField($field){
        return "<div {$this->joinAttributes($this->attrsBlock[$field])}><label for='id_{$field}'>{$field}</label>{$this->fieldForm[$field]}</div>";
    }
    ...
    private function cleanData($data){
        return $this->DB->cleanData($this->table,$data);
    }
    ...
    public function setFieldAttributes(string $field, array $attributes) {
        if (isset($this->_fields[$field])) {
            $this->attrs[$field] = array_merge($this->attrs[$field] ?? [], $attributes);
            $this->fieldForm[$field] = $this->generateInputField($this->attrs[$field], strtok($this->_fields[$field]->Type,'('), $this->isRequired($this->_fields[$field]->Null));
            $this->fieldFormBlock[$field] =$this->generateBlockField($field,$this->attrsBlock[$field]);
        }
    }

    // Récupère les attributs des blocs de champs
    public function getFieldBlockAttributes(string $field = null) {
        return $field ? ($this->attrsBlock[$field] ?? null) : $this->attrsBlock;
    }

    // Ajoute ou modifie les attributs d'un bloc de champ
    public function setFieldBlockAttributes(string $field, array $attributes) {
        if (isset($this->_fields[$field])) {
            $this->attrsBlock[$field] = array_merge($this->attrsBlock[$field] ?? [], $attributes);
            $this->fieldFormBlock[$field] =$this->generateBlockField($field);
        }
    }

    // recuperer les champs des inputs
    public function getFieldForm(string $field = null){
        return $field ? ($this->fieldForm[$field] ?? null) : $this->fieldForm;
    }

        // recuperer les champs des inputs
    public function getFieldBlockForm(string $field = null){
        return $field ? ($this->fieldFormBlock[$field] ?? null) : $this->fieldFormBlock;
    }
    ...
    private function fetchWithoutJoins(): array {
        return (array) $this->all($this->table);
    }
    ...
    public function generateHtmlTable(array $params=NULL): string {
        ...
        $data=$params != NULL ? $params :$this->DB->all($this->table)->data;
